...
// bald-service-id: define the ID of the service
// bald-service-app: define the name of the service
// bald-service-version: define the version of the service
// bald-service-metadata: define the metadata of the service
// bald-service-protocols: define the protocols of the service
//
// Example Deployment:
/*
apiVersion: apps/v1
...
template:
  metadata:
    labels:
      app: nginx
      bald-service-id: "56991810-c77f-4a95-8190-393efa9c1a61"
      bald-service-app: "nginx"
      bald-service-version: "v3.5.0"
    annotations:
      bald-service-protocols: |
        {"80": "http"}
      bald-service-metadata: |
...
const (
	...
	AnnotationsKeyProtocolMap = "bald-service-protocols"
)
...
// New 按 in-cluster 配置自建 clientset 并构造 Registry（拥有连接生命周期，
// Close 时停止 informer）。集群外使用请改用 NewWithClient 注入 clientset。
func New(opts ...Option) (*Registry, error) {
	cfg, err := rest.InClusterConfig()
	if err != nil {
		return nil, fmt.Errorf("kubernetes: load in-cluster config: %w (不在集群内请用 NewWithClient 注入 clientset)", err)
	}
	cs, err := kubernetes.NewForConfig(cfg)
	if err != nil {
		return nil, fmt.Errorf("kubernetes: create clientset: %w", err)
	}
	return NewWithClient(cs, opts...), nil
}

// NewWithClient 注入既有 clientset 构造 Registry（不负责关闭）。
func NewWithClient(clientSet *kubernetes.Clientset, opts ...Option) *Registry {
	o := newOptions(opts...)
