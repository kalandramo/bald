...
| 后端实现数量 | 自研 1 种（`inmemory`）+ 直连 provider（contrib/registry，移植 go-wind） | ❌ 仅接口 | ✅ etcd/consul/nacos 等（官方插件）| ✅ 8 种（consul/etcd/eureka/kubernetes/nacos/polaris/servicecomb/zookeeper，插件化）|
| `inmemory` 实现 | ✅（**自研**，测试/本地）| ❌ | ❌ | ❌ |
| 直连 SDK 后端 | ✅ etcd/nacos/consul/kubernetes（`contrib/registry/<backend>` 独立 module + 契约装配）| ❌ | — | ✅（plugins/registry 同款隔离）|
...
2. **直连 provider、单向依赖**
   后端接入收敛在 `contrib/registry/<backend>` 独立 module（根包纯 SDK 实现，`contract` 子包做契约映射），核心只依赖 `pkg/registry` 契约。无反向依赖（不要求具体后端知道 bald 的存在）。
...
**真正自研（三方都没有或不全）：**
1. `pkg/registry` 自研 `Registrar` + **`inmemory`** + 动态端口注册 + 契约装配（`RegistrarRegistry` 显式注册 + contrib/registry 直连 provider；go-lulu 注册后端齐全但无 inmemory；onexstack 仅接口无动态端口；Kratos 无 inmemory）
...
| 注册中心 | 直连 provider + 契约装配 | ✅ | `contrib/registry/{etcd,nacos,consul,kubernetes}` 独立 module（移植 go-wind）|
...
- 实现：`inmemory`（测试/本地）、`contrib/registry/<backend>` 直连 provider（etcd/nacos/consul/kubernetes，契约装配走 `appkit.RegistrarRegistry`）。
...
- [x] **proto 配置契约**：`pkg/conf` 全栈 proto 化，`pkg/options` 中间层已删除（决策记录与坑位收编至 [Bald 配置系统设计](./Bald%20配置系统设计.md) §2）。
