...
- [服务注册设计](./服务注册设计.md)：registry 模块（2026-09-17 自 `pkg/registry` 下放为顶层独立 module）——Registrar/Discovery/Watcher 抽象、inmemory 实现、`registry/<backend>` 直连 provider 与契约装配。
- [服务注册模块下放设计](./服务注册模块下放设计.md)：registry 契约下放为顶层独立 module、后端自 `contrib/registry/*` 上移 `registry/<backend>` 的设计论证——依赖隔离收益、breaking 代价与影响面、迁移与发版步骤。
