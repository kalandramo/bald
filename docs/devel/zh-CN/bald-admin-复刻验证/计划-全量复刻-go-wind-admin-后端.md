...
| **5** | 审计字典存储 + 编码/存储轴 | ✅ 完成（5.1–5.6） | `4384808`…`277b107` |
| **6** | 管理面聚合 API（admin 域 162 rpc） | ✅ 路线甲完成（B 类 4 rpc；A 类 31 跳过） | `69a63f8` |
| **7** | 前端对齐与收尾 | ✅ 完成（9 新域页面 + 视觉终验 `3b89aa2`） | — |

### 缺陷报告

`bald-framework-defects-D1-D15.md`（同目录）——**19 条**：10 条已修、1 条部分已修、
9 条待上游确认（截至 2026-09-24）。
...
1. 环境重建见 `交接文档-跨设备续作.md`「环境与命令约定」（**Windows 设备先读「Windows 设备环境」章**）；
...
**`bald-framework-defects-D1-D15.md`**（同目录）——D1–D19 的完整报告，
每条附可复核的 file:line 锚点 + 可直接执行的复现命令。截至 2026-09-24 为
**10 条已修、1 条部分已修、9 条待上游**（D1–D19）。报告附「核验方法」章节，记录了锚点漂移与自身修正。
...
#### 6.2 执行记录（2026-09-20，路线甲）

**用户确认走路线甲**：只做 B 类真新增，A 类 31 个包装类跳过。

| # | 任务 | 状态 |
|---|---|---|
| 6.2 | (3 rpc) + (1 rpc) | ✅ （4 rpc 全实现，e2e 全绿） |
| 6.3 | A 类 31 个包装类 | ⏭️ **按路线甲跳过**（业务能力已由 Wave 1–5 biz 层覆盖） |

**实测发现（授权回归）**：首版给 viewer 开  权限点——但
（管理面组件目录，admin 专属）经  也
归一化为 ，直接放开了它（ 由 403 变 200）。
改用**自定义 ObjectResolver** 产出独立 object  隔离两者。
**教训**：P9 的  取路径首段，同一前缀下的不同语义路由会撞名。

**未移植项**：源的 （套餐白名单过滤菜单）——本项目
 的 module 字段语义未对齐，强行实现会造无数据来源的逻辑。

> **称量（不做 A 类的代价）**：若跳过 A 类，「388 rpc 全量对等」的字面目标有缺口；
> 但 A 类的**业务能力**已 100% 覆盖（同 biz 层），缺口仅在 API 路径面。
> **用户裁定（2026-09-20）：A 类缺口记录为「已知缺口」，等 Wave 7 后再议**——
> 当前不做路径面补全。
