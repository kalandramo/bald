...
// staticReader 是一个简易的 [Reader]，始终返回固定数据。
...
// swapReader 是线程安全的 [Reader]，它的返回值支持动态变更。
...
// closeTracker 封装 [staticReader]，用于记录 Close 方法是否被调用过。
...
// closeErrReader 是实现 [Closer] 的类型，Close 调用时总是返回错误。
...
// stubValueWatcher 是 [ValueWatcher] 的桩实现，从由调用方控制的通道向外推送配置值。
...
// watchErrReader 实现 [ValueWatcher] 接口，但 WatchValue 调用总是返回错误。
