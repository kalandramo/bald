...
位置参数说明：
  ...
  oncall redis "主从切换失败" --level 1`,
		...
		// cobramcp.AnnotationArgPrefix + "<index>" 为每个位置参数提供语义描述。
		// index 从 0 开始，与 cmd.Use 中 token 的出现顺序对应。
		Annotations: map[string]string{
			// 第 0 个 token：<module>
			cobramcp.AnnotationArgPrefix + "0": "值班模块名称（如 bke、kafka、redis、mysql）。" +
				"决定工单路由到哪个团队的值班人员。",
			// 第 1 个 token：[title]
			cobramcp.AnnotationArgPrefix + "1": "工单标题，简要描述问题现象（可选）。" +
...
两个位置参数均为必填项，缺少任意一个都会报错。`,
  ...
  cp --recursive /data/mysql/ /backup/mysql/`,
		...
		Annotations: map[string]string{
			cobramcp.AnnotationArgPrefix + "0": "源文件或目录的路径（必填）。支持绝对路径和相对路径。",
			cobramcp.AnnotationArgPrefix + "1": "目标文件或目录的路径（必填）。目标目录须已存在。",
		},
...
[targets...] 为可选的变参列表：
  ...
  make --dir /opt/app build`,
		...
		Annotations: map[string]string{
			cobramcp.AnnotationArgPrefix + "0": "要执行的 Makefile 目标列表（可选）。" +
...
func newKubectlGetCmd() *cobra.Command {
  ...
  kubectl get deployment my-app --namespace production --output yaml`,
		...
		Annotations: map[string]string{
			cobramcp.AnnotationArgPrefix + "0": "Kubernetes 资源类型（必填）。" +
				"常见值：pod、deployment、service、configmap、secret、node。",
			cobramcp.AnnotationArgPrefix + "1": "资源名称（可选）。" +
...
//
// [flags] is the cobra sentinel and is silently ignored by cobramcp.
...
func newKubectlLogsCmd() *cobra.Command {
  ...
  kubectl logs my-app-pod --follow`,
		...
		Annotations: map[string]string{
			cobramcp.AnnotationArgPrefix + "0": "Pod 名称（必填）。" +
...
func newKubectlExecCmd() *cobra.Command {
  ...
  kubectl exec my-app-pod --container sidecar cat /etc/config.yaml`,
		...
		Annotations: map[string]string{
			cobramcp.AnnotationArgPrefix + "0": "目标 Pod 名称（必填）。",
			cobramcp.AnnotationArgPrefix + "1": "在容器内执行的命令及其参数（必填，变参）。" +
...
// ─────────────────────────────────────────────────────────────────────────────
// New Pattern – chat command with no args and a --test flag
//
// cmd.Use:  "chat"
//
// Resulting positional_args JSON Schema:
...
func newChatCmd() *cobra.Command {
	var isTest bool // 用于存储 --test flag 的值

	cmd := &cobra.Command{
		Use:   "chat",
		Short: "启动聊天会话",
		Long: `此命令用于启动一个交互式聊天会话。

  它没有任何位置参数。

  使用 --test flag 可以启用测试模式。`,
		Example: `  # 启动普通聊天会话
  yourcli chat
  ...
  yourcli chat --test`,
		...
		Args: cobra.NoArgs, // chat 命令没有任何位置参数

		Annotations: map[string]string{
			// cobramcp.AnnotationArgPrefix + "0": "没有位置参数", // NoArgs 时不需要此注解
		},

		RunE: func(cmd *cobra.Command, args []string) error {
			if isTest {
				fmt.Fprintf(cmd.OutOrStdout(), "Chat session started in TEST mode.\n")
			} else {
				fmt.Fprintf(cmd.OutOrStdout(), "Chat session started in normal mode.\n")
			}
			// 这里可以添加实际的聊天会话逻辑
			return nil
		},
	...
	// 添加 --test flag
	cmd.Flags().BoolVar(&isTest, "test", false, "启用测试模式")
	return cmd
}
...
var rootCmd = &cobra.Command{
	Use:   "yourcli",
	Short: "一个示例命令行工具",
	Long: `这是一个使用 Cobra 和 Cobrax 构建的示例命令行工具，
演示了不同类型的位置参数和 flag 的配置。`,
}
...
func init() {
	...
	rootCmd.AddCommand(newChatCmd()) // 添加新的 chat 命令
}
*/
