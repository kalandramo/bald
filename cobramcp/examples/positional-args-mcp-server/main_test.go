...
// newTestFactory returns a factory function for use in tests.
// Each call to the returned factory produces a fresh command tree.
func newTestFactory() func() *cobra.Command {
	return func() *cobra.Command {
		root := buildRootCmd()
		root.AddCommand(cobramcp.Command(nil))
		return root
	}
}
...
func TestOncallSchema(t *testing.T) {
	srv, err := cobramcp.NewMCPServer(cobramcp.MCPOptions{Enabled: false, Name: "myops"}, newTestFactory())
...
func TestChatSchema(t *testing.T) {
	...
	// chat 没有位置参数 → 不出现 args / flags / positional_args 包装对象。
	assert.NotContains(t, schema.Properties, "args",
		"chat: args must be absent when the command has no positional arg tokens")
	assert.NotContains(t, schema.Properties, "flags",
		"chat: flags must be flat")
	assert.NotContains(t, schema.Properties, "positional_args",
		"chat: positional_args must never exist")

	// --test 标志仍必须是顶层属性。
	require.Contains(t, schema.Properties, "test")
	assert.Equal(t, "boolean", schema.Properties["test"].Type)
}
