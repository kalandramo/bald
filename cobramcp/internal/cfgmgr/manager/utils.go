...
// GetCmdPath builds the command path to the cobrax command.
...
func GetCmdPath(cmd *cobra.Command, commandName string) ([]string, error) {
	...
	// Find the index of the cobrax command in the command path
	index := slices.Index(args, commandName)
	if index == -1 {
		return nil, fmt.Errorf("command %q not found in path %q", commandName, path)
	}

	// Return the slice from after the root command to the cobrax command
