...
func AddFlagToSchema(schema *jsonschema.Schema, flag *pflag.Flag) {
		...
		if flag.Annotations != nil {
			if schemaStrArray, ok := flag.Annotations["jsonschema"]; ok {
				if len(schemaStrArray) == 0 {
					baldlog.Warn(nil, fmt.Sprintf("No value for jsonschema annotation for flag %s, treating as type string", flag.Name))
					flagSchema.Type = "string"
				} else {
					var aSchema jsonschema.Schema
					err := aSchema.UnmarshalJSON([]byte(schemaStrArray[0]))
					if err != nil {
						baldlog.Error(nil, fmt.Sprintf("Error when decoding JSON schema for flag %s (%v), treating as type string", flag.Name, err))
						flagSchema.Type = "string"
					} else {
						flagSchema = &aSchema
					}
				...
				baldlog.Debug(nil, fmt.Sprintf("No annotation called jsonschema for flag %s, treating as type string", flag.Name))
				flagSchema.Type = "string"
			}
		} else {
			flagSchema.Type = "string"
	...
	default:
		...
		baldlog.Debug(nil, "unknown flag type, defaulting to string", "flag", flag.Name, "type", t)
	}
