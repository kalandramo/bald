...
func parseArray(defValue string, schema *jsonschema.Schema) any {
	...
	if !strings.HasPrefix(defValue, "[") || !strings.HasSuffix(defValue, "]") {
		baldlog.Warn(nil, "malformed array default value: must start with '[' and end with ']'", "value", defValue)
		return nil
	}
	...
	default:
		baldlog.Warn(nil, "unsupported array item type for default value", "type", schema.Type, "value", defValue)
		return nil
	}
}
...
func parseIntArray(parts []string) []int64 {
	...
	for i, p := range parts {
		...
		if val, err := strconv.ParseInt(trimmed, 10, 64); err == nil {
			...
			baldlog.Warn(nil, "skipping invalid integer in array default value",
				"index", i,
				"value", p,
				"error", err)
		}
...
func parseFloatArray(parts []string) []float64 {
	...
	for i, p := range parts {
		...
		if val, err := strconv.ParseFloat(trimmed, 64); err == nil {
			...
			baldlog.Warn(nil, "skipping invalid float in array default value",
				"index", i,
				"value", p,
				"error", err)
		}
...
func parseBoolArray(parts []string) []bool {
	...
	for i, p := range parts {
		...
		if val, err := strconv.ParseBool(trimmed); err == nil {
			...
			baldlog.Warn(nil, "skipping invalid boolean in array default value",
				"index", i,
				"value", p,
				"error", err)
		}
...
func parseObject(defValue string, schema *jsonschema.Schema) any {
	...
	default:
		baldlog.Warn(nil, "unsupported object item type for default value", "type", schema.Type, "value", defValue)
		return nil
	}
}
...
func parseIntObj(parts []string) map[string]int64 {
	...
	for i, p := range parts {
		...
		if len(split) != 2 {
			baldlog.Warn(nil, "malformed flag default value object", "value", p)
			continue
		}
		...
		if val, err := strconv.ParseInt(valStr, 10, 64); err == nil {
			...
			baldlog.Warn(nil, "skipping invalid integer obj in array default value",
				"index", i,
				"value", p,
				"error", err)
		}
