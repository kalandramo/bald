...
- 内置命令：cobramcp 自身的 `mcp`（或 `Config.CommandName`）、`help`、`completion`。
...
cmd.Annotations = map[string]string{
    cobramcp.AnnotationArgPrefix + "0": "值班模块名称（如 bke、kafka、redis）…",
    cobramcp.AnnotationArgPrefix + "1": "工单标题（可选），省略时自动生成…",
}
```
...
- **args** = `GetCmdPath(cmd, "mcp")` 的结果 + `"start"` —— `GetCmdPath`（`manager/utils.go:31`）从**当前命令路径**中定位 cobramcp 命令段，使 cobramcp 命令挂在任意子命令层级下仍能正确重建启动路径；
...
rootCmd.AddCommand(cobramcp.Command(nil))   // 注入 mcp start/stream/tools/rest + 编辑器管理
err := rootCmd.Execute()
```
...
srv, _ := cobramcp.NewMCPServer(cobramcp.MCPOptions{
    Enabled: true, Addr: ":8090", Name: "myapp", Version: "1.0.0",
}, factory, cobramcp.WithSelectors(mySelectors...))
srv.Start(ctx)
```
...
2. `cobramcp.AnnotationArgPrefix + "<index>"` 为每个位置参数写语义描述；
