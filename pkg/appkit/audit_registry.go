...
// import，主程序在 main() 里逐个 MustRegister）；段存在但 type 为空/未注册
// fail-fast；后端 cleanup 挂 Effect 逆序回放。audit 段按 type **单选**分发
// （契约语义：audit.type ∈ {"log", "store", "stream"}）。
//
// 与 tracer/metrics 的差异：type=log **内置**于 Build——LoggerAuditor 在
// 核心包零依赖，appkit import 无负担，业务零注册即得（对齐 codegen
// buildAuditBackend 的 log 开箱即用语义）。store/stream 走 contract 注册
...
// AuditRegistry 是审计后端 Provider 的显式注册表（audit.type 单选；
// type=log 内置于 Build，无需注册）。
...
// Build 按契约 audit.type 单选分发。段为 nil → 不装配（nil, nil, nil）；
// type 为空/未注册均 fail-fast（与 TracerRegistry 同款主开关语义）；
// type=log 内置返回核心 LoggerAuditor（无需注册）。
func (r *AuditRegistry) Build(ctx context.Context, cfg *bootstrapv1.Audit) (audit.Auditor, func(context.Context) error, error) {
	...
	typ := cfg.GetType()
	if typ == "" {
		return nil, nil, errors.New(`appkit: audit.type is required when audit section is present (expect "log", "store" or "stream")`)
	}
	if typ == AuditTypeLog {
		return audit.NewLoggerAuditor(), nil, nil // 内置：零依赖开箱即用
	}
	r.mu.Lock()
...
type auditState struct {
	...
	prev audit.Auditor
	// cleanup 后端资源释放（如 stream 停机 flush 尾批）。
	cleanup func(context.Context) error
}

// buildAudit 在阶段 B（契约装载校验后）按 audit 段装配审计后端并注入
// 全局。段缺失为 no-op（全局保持现状）；type=log 内置装配（无需
// Registry）；type=store/stream 需对应 Registry 已接线，否则 fail-fast
// （配置意图无法兑现）。返回停机句柄，由 FromBootstrap 挂 Effect。
func buildAudit(cfg *bootstrapv1.BootstrapConfig, spec *bootstrapSpec) (*auditState, error) {
	...
	typ := acfg.GetType()
	if typ == "" {
		return nil, errors.New(`appkit: audit.type is required when audit section is present (expect "log", "store" or "stream")`)
	}
	...
	if typ == AuditTypeLog {
		a = audit.NewLoggerAuditor() // 内置：零依赖开箱即用
	} else {
		if spec.auditRegistry == nil {
			return nil, errors.New("appkit: bootstrap.audit present but no AuditRegistry wired (WithAuditRegistry missing)")
		}
		var err error
		a, cleanup, err = spec.auditRegistry.Build(context.Background(), acfg)
		if err != nil {
