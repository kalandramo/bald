...
type AuditEvent struct {
	...
	//
	// **空值语义（消费方务必阅读）**：空串表示「本事件无法确定归属租户」，
	// 不是「平台级事件」。有三类埋点天然拿不到租户：
	//   - 认证失败（用户不存在 / 限流 / 熔断——无从得知其所属租户）；
	//   - 权限变更（协调器埋点，无请求上下文）；
	//   - 数据访问（ORM 插件采集，事件里无租户）。
	//
	// 这些恰是审计最需要的线索（谁在试密码 / 谁改了权限 / 谁访问了敏感表）。
	// **落库实现不应丢弃它们**，而应统一兜底到一个可检索的归属值——
	// 兜底策略属应用决策（bald-admin 用 `audit.fallback_tenant` 配置），
	// 框架层不预设具体值。若落库后端按租户隔离读取，空值事件会查不到，
	// 故兜底应在**落库汇聚点**（如 RecordMapper）统一完成，一处覆盖全部调用方。
	TenantID string
