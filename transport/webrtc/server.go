...
func (s *Server) listenAndEndpoint() error {
	...
	if s.endpoint == nil {
		...
		if err != nil || port == "" {
			s.endpoint = &url.URL{Scheme: scheme, Host: addr, Path: s.path}
		} else {
			if host == "" || host == "0.0.0.0" {
				host = "localhost"
			}
			s.endpoint = &url.URL{Scheme: scheme, Host: net.JoinHostPort(host, port), Path: s.path}
		}
	}
