...
本模块实现了 `go-wind/transport.Server` 接口，支持多命名流、订阅者管理、事件重放（Last-Event-ID）和可选鉴权。由于 SSE 基于 HTTP，服务器包装了标准 `http.Server`，支持与 `transport/http` 相同的中间件类型，可以与 `go-wind` 应用框架无缝集成。
